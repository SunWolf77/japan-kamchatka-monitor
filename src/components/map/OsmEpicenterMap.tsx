...
import { colourHex as kvertColourHex, type KvertVolcanoStatus } from "@/lib/seismic/kvert";
import { fetchKvertAlerts } from "@/lib/seismic/kvertServer";
import { colorHex as usgsColourHex, type UsgsVolcanoAlert } from "@/lib/seismic/usgsVolcano";
import { fetchUsgsVolcanoAlerts } from "@/lib/seismic/usgsVolcanoServer";
import { volcanoesForNode } from "@/lib/seismic/volcano-watch";
...
export function OsmEpicenterMap({
  ...
  const volcanoRef = useRef<import("leaflet").LayerGroup | null>(null);
  const kvertRef = useRef<KvertVolcanoStatus[]>([]);
  const usgsElevRef = useRef<UsgsVolcanoAlert[]>([]);
  ...
  // KVERT + USGS elevated pins (Kamchatka node)
  useEffect(() => {
    if (node.id !== "kamchatka") return;
    let cancelled = false;
    void (async () => {
      try {
        const [kv, us] = await Promise.all([
          fetchKvertAlerts().catch(() => null),
          fetchUsgsVolcanoAlerts().catch(() => null),
        ]);
        if (cancelled) return;
        kvertRef.current = kv?.elevated ?? [];
        usgsElevRef.current = (us?.elevated ?? []).filter(
          (v) => v.lat != null && v.lon != null && (v.isAvo || v.isPacificNeighbor),
        );
        void redrawVolcanoPins();
      } catch {
        /* static only */
  ...
  async function redrawVolcanoPins() {
    const L = await import("leaflet");
    const map = mapRef.current;
    if (!map) return;
    if (volcanoRef.current) {
      map.removeLayer(volcanoRef.current);
      volcanoRef.current = null;
    }
    const group = L.layerGroup();
    for (const v of volcanoesForNode(nodeRef.current.id)) {
      L.circleMarker([v.lat, v.lon], {
        ...
        .bindTooltip(
          `<div style="font:11px/1.35 system-ui"><strong>${v.name}</strong><br/>watchlist · ${v.region}</div>`,
          { sticky: true, opacity: 0.95 },
        )
        .addTo(group);
    }
    for (const v of kvertRef.current) {
      if (v.lat == null || v.lon == null) continue;
      const fill = kvertColourHex(v.colour);
      L.circleMarker([v.lat, v.lon], {
        radius: 8,
        color: "#fff",
        weight: 1.5,
        ...
        .bindTooltip(
          `<div style="font:11px/1.35 system-ui"><strong>${v.name}</strong> · KVERT ${v.colour}` +
            (v.synopsis ? `<br/>${v.synopsis.slice(0, 160)}` : "") +
            `<br/><span style="opacity:.75">KVERT · not USGS</span></div>`,
          { sticky: true, opacity: 0.95 },
        )
        .addTo(group);
    }
    for (const u of usgsElevRef.current) {
      if (u.lat == null || u.lon == null) continue;
      L.circleMarker([u.lat, u.lon], {
        radius: 7,
        color: "#fff",
        weight: 1.2,
        fillColor: usgsColourHex(u.colorCode),
        fillOpacity: 0.85,
      })
        .bindTooltip(
          `<div style="font:11px/1.35 system-ui"><strong>${u.name}</strong> · USGS ${u.obs.toUpperCase()} ${u.colorCode}` +
            `<br/><span style="opacity:.75">U.S. volcano — not KVERT</span></div>`,
          { sticky: true, opacity: 0.95 },
        )
        .addTo(group);
    }
    group.addTo(map);
    volcanoRef.current = group;
  }
